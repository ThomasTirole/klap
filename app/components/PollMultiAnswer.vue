<template>
  <div class="space-y-6">
    ...
    <div class="text-center">
      ...
      <p v-if="config.maxChoices" class="text-sm text-indigo-600 font-medium mt-2">
    ...
    <div v-else>
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div class="space-y-3">
          ...
          >
            ...
            <div
              ...
              :class="{
                'border-indigo-600 bg-indigo-50': selectedOptions.includes(option.id),
                'border-gray-300 hover:border-indigo-400': !selectedOptions.includes(option.id),
                'opacity-50 cursor-not-allowed': isMaxReached && !selectedOptions.includes(option.id)
              }"
            >
              <div class="flex items-center justify-between">
                <span class="text-lg font-medium text-gray-900">{{ option.label }}</span>
                <div
                  class="w-6 h-6 border-2 rounded transition-all flex items-center justify-center"
                  :class="{
                    'border-indigo-600 bg-indigo-600': selectedOptions.includes(option.id),
        ...
        <p v-if="config.maxChoices" class="text-sm text-center text-gray-600">
          {{ selectedOptions.length }} / {{ config.maxChoices }} choix sélectionné{{ selectedOptions.length > 1 ? 's' : '' }}
        </p>
...
import type { Item, PollMultiConfig, PollMultiPayload } from '~/types/database'
...
const isMaxReached = computed(() => {
  if (!config.value.maxChoices) return false
  return selectedOptions.value.length >= config.value.maxChoices
})
...
const handleSubmit = async () => {
  ...
  try {
    const payload: PollMultiPayload = {

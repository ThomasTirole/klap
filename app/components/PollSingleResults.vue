...
import type { Item, PollSingleConfig, PollSinglePayload, Response } from '~/types/database'
...
const handleNewResponse = (response: Response) => {
  const payload = response.payload as PollSinglePayload
  if (payload.optionId) {
    results.value[payload.optionId] = (results.value[payload.optionId] || 0) + 1
...
const setupSubscription = () => {
  ...
  // Se désabonner de l'ancien channel si existant
  if (realtimeChannel) {
